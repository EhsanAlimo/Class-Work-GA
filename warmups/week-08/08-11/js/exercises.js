...
const pairwise = function (array, num) {
  ...
  array.forEach((element, index) => {
    ...
    if (indexNum >= 0) {
      //indexNum is -1 there is no corresponding value
      if (!indexList.includes(indexNum) && !indexList.includes(index)) {
        //takes of the duplicate indices
    ...
    secondArray.forEach((element2, index2) => {
      ...
      if (element2 === value) {
        //once the loop finds where the element2 threequals value, push its indices as its own array into the pairs array
